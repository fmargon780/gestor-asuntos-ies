...
| `indice-archivo.json` | Desde la fila 177 (26-sep-2026, `docs/ARCHIVO-POR-CURSO-Y-RUTAS.md`), es solo el RESUMEN: `{ version, hechoEl, cursos: ['2026-27', '2025-26', …], recuento: { CATEGORIA: nº de carpetas de tercero } }`. La lista de verdad vive partida por curso académico (ver la fila de abajo). Un índice viejo (con `asuntos` dentro) se migra solo la primera vez que se lee: se parte por curso, y el fichero viejo se aparta a `copias/indice-archivo-antiguo-AAMMDD.json`. **Fuera de los dieciocho**, por el mismo motivo que `presencia.json`: se puede rehacer entero en cualquier momento con "Reconstruir el índice", así que no necesita copia de seguridad, papelera ni fusión de conflictos (`js/conflictos.js` se salta toda la carpeta `indice-archivo/`). Se escribe y relee directo con `Carpetas` |
| `indice-archivo/<curso>.json` | Fila 177: `{ version, hechoEl, hechoPor, recuento, asuntos: [{ nombre, categoria, tercero, ruta, fecha, tipo, curso, grupo, documentos, registros, sueltoEn }] }`, uno por curso académico (`2025-26`, `2026-27`…; del 1 de septiembre al 31 de agosto, de la fecha AAMMDD del nombre de la carpeta). `IndiceArchivo.leerDisco()` sin opciones lee solo el curso actual; `{curso:'2025-26'}`, ese; `{todos:true}`, todos juntos (lo usan Cuentas, fichas huérfanas, sugerencias de "Por clasificar", "Repartir un PDF" y el plazo de conservación). `anadirEntrada`/`quitarEntrada` solo reescriben el fichero de SU curso: archivar un asunto más no reescribe los demás. Mismo trato que el resumen: fuera de los dieciocho, sin copia ni papelera (`js/archivo-indice.js`, ver "El índice del ARCHIVO") |
| `rutas.json` | `{ abiertos, archivo }`: dónde están las dos carpetas DENTRO de Dropbox, con `/`, igual para los dos ordenadores (fila 161, `js/copiar-ruta.js`, botón «Ruta»). Pequeño, como `margenes-pdf.json`: se relee antes de guardar y pasa por `Copias.guardar`. Desde la fila 177, también lo usa `Nombres.topes()` (`js/nombres-topes.js`) para saber cuánto ocupa la ruta hasta el ARCHIVO al calcular el hueco para un nombre; se precarga al entrar (`RutaCarpetas.cargarComun()`) |
...
`borrados-listas.json` y, desde la fila 177, cada fichero de `indice-archivo/<curso>.json` por
separado (la clave de la cola es `'indice-archivo:' + curso`: archivar en cursos distintos no se
espera entre sí). Nunca se llama a `poner` del
