...
| `copias/*.json` | Copias de seguridad de los nueve ficheros de arriba, una por día, 30 como mucho de cada uno |
...
- `js/copias.js` guarda, antes de escribir cualquiera de los nueve ficheros compartidos
  (`asuntos.json`, `guias.json`, `tipos.json`, `estados.json`, `tipos-documento.json`,
  `tablon.json`, `recurrentes.json`, `frescura.json`, `campos.json` desde el 11-sep-2026), una
...
- Todo lo que escribe uno de esos nueve ficheros llama a `Copias.guardar` en vez de a
  `Carpetas.guardarJson` directamente.
- Al pulsar Entrar se comprueban los nueve ficheros (`Copias.comprobarTodos`). Si alguno está
...
- Los demás (`tipos.json`, `estados.json`, `tipos-documento.json`, `guias.json`,
  `recurrentes.json`, `frescura.json`, `campos.json`) cambian mucho menos y no se fusionan solos: salen en el
...
- **Meter los campos de cada tipo en el nombre de los documentos.** Son del asunto, no del
  papel (11-sep-2026).
