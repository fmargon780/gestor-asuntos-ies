...
| `js/hitos-anadir.js` | «Añadir documento» dentro de un hito (fila 103): menú con «Desde el ordenador», «Desde «Por clasificar»» y «Uno que ya está en la carpeta»; `HitosAnadir.apuntar` deja lo añadido apuntado a ese hito |
| `js/hitos-documento-menu.js` | Los tres puntos de cada documento de un hito (fila 103): Registrar, las herramientas de PDF y «Quitar del hito»; lo nuevo que salga se apunta al mismo hito (`HitosDocumentoMenu.nuevos`) |
...
| `js/plantillas.js` | Leer y guardar `plantillas.json`, montar `Plantillas.valoresDeAsunto` y rellenar los huecos: el motor, sin pantalla. El hueco `{{LO QUE FALTA}}`, con doble llave, se sustituye aparte y siempre (`tieneLoQueFalta`, fila 59); desde la fila 81 también `{{FIRMANTE}}` y compañía (con el cargo vigente en la fecha del documento) y `{{CONSEJERIA}}` |
...
| `pruebas/el-hito-mesa-de-trabajo.mjs` | Prueba (fila 103, 23-sep-2026): la resta de ficheros nuevos, cuándo sale cada botón del hito, los adjuntos ya marcados, la constancia con documentos, y en la ficha el menú de «Añadir documento», meter un suelto, el menú de tres puntos (Registrar, Quitar del hito) y «Comunicar» sin texto propio |
