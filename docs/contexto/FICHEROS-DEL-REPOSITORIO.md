...
| `pruebas/envio-apps-script.mjs` | Prueba (con `vm`, fila 115): `doPost`/`enviarCorreo`/`hiloParaResponder`/`prepararEnvio` de `apps-script/gestor-correos.gs`, con `GmailApp`/`PropertiesService`/`Utilities` de mentira |
| `pruebas/correo-enviar.mjs` | Prueba (con `vm`, fila 115): `leerUrl`/`guardarUrl`/`enviar`/`probar` de `js/correo-enviar.js`, con `fetch`/`localStorage` de mentira |
