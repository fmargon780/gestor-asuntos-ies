...
(`js/hitos-documentos.js`, "Apuntar un documento"), "Comunicar", «+ Añadir documento», y "Meter en un
...
- **«+ Añadir documento»** (fila 168, `js/ficha-documentos.js`, `ponerBotonAnadir(bloqueEl, a)`;
  sustituye a «Documentos ▾», que ya no está en la ficha): `App.verDocumentos(a, { irDirectoAAnadir:
  true })`, el mismo cuadro de siempre yendo directo al selector de fichero, en la cabecera del
  propio bloque; se pinta también con la carpeta vacía, comprobando que no exista ya (el título no
  se rehace en cada repintado). La ventana de `App.verDocumentos` se queda para la tarjeta de la
  lista, «Por clasificar» y «Nuevo asunto». Cada fila de documento: nombre, **⧉** detrás (copia sin
  extensión, `Copiar.boton`; la marca `data-con-copiar` hace que `js/copiar.js` no meta su
  «Copiar»), «Sin registrar»/«Registrar», **«Poner nombre»** siempre (`App.verDocumentos(a,
  { ponerNombre })`, no en el índice del expediente), «Asociar a un hito» y ⋮ con solo «Pasar a
  versiones previas» y «Borrar». Se comprueba con `pruebas/documentos-en-un-solo-sitio.mjs`.
