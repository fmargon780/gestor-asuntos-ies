...
- **Crear o editar la plantilla desde el propio cuadro** (25-sep-2026, fila 151,
  docs/PLANTILLA-DESDE-EL-CUADRO.md): en el cuadro de Correo y en el de Séneca, junto al
  desplegable «Plantilla». Sin ninguna del tipo, un botón «Crear plantilla»; con alguna, «Editar
  plantilla» (edita la elegida). El editor se abre **dentro del mismo cuadro**, nunca con un
  segundo `U.preguntar`: mismo patrón que `#correo-formulario`/`#correo-resumen`
  (`js/correo-cuadro.js`) — un bloque hermano (`#correo-plantilla-editor` /
  `#seneca-plantilla-editor`) que se enseña y esconde la clase `oculto`, sin perder «Para», el
  asunto ni los documentos marcados. El tipo y la categoría son los del asunto abierto, sin
  preguntarlos. El editor en sí (`PlantillasAjustes.montarEditorEnLinea`, `js/plantillas-ajustes.js`)
  reutiliza `campoDeTextoHTML`/«Insertar hueco» del editor de Ajustes, con la vista previa de los
  datos reales del asunto en el que se está (`Plantillas.valoresDeAsunto`), sin duplicar el editor.
  Al guardar: `Plantillas.guardar` (releyendo, compartido con el compañero),
...
Se comprueba con `pruebas/plantillas.mjs` y, para el editor en línea, `pruebas/plantilla-desde-el-cuadro.mjs`.
El bloque de pantalla vive en `js/plantillas-ajustes.js`
