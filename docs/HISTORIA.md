...
Fila 26 de la cola (`docs/COLA.md`, `docs/HITOS-SON-LA-GUIA.md`). Acordado con Francisco nada
más ver en pantalla el resultado de la fila 15 (`docs/HITOS.md`): dos zonas, la guía como texto
con casillas y los hitos como un añadido aparte debajo, cuando él solo había pedido una cosa: que
los pasos de la guía FUERAN los hitos.

**Qué cambia.** Al abrir la ficha de un asunto abierto cuyo tipo tiene guía y que todavía no
tiene hitos, se crean solos, sin botón ni pregunta (`Hitos.crearDesdeGuiaImportando`, ya escrita
en la fila 15, respetando `pasosHechos`/`pasosElegidos`). La guía deja de pintarse como texto con
casillas dentro de la ficha; el bloque pasa a llamarse "Hitos", con el enlace de escribir o
cambiar la guía al final, y "+ Añadir un hito" (o "+ Añadir el primer hito", si el tipo no tiene
guía) donde antes estaba "Crear los hitos de la guía". Desaparecen ese botón y el bloque
`#hitos-entrada`.

**El candado.** El observador de `js/hitos-panel.js` repinta cada 30 ms y `repintar()` es
`async`: sin cuidado, dos pasadas podrían colarse antes de que `hitos.json` terminara de
escribirse y crear los hitos dos veces. Se ha puesto un candado por clave de asunto
(`creandoParaClave`); no bastaba con mirar cuántos hitos había ya, porque las dos pasadas podían
leer el mismo "todavía ninguno" antes de que la primera terminara de guardar. Tampoco se crean si
el asunto está archivado, si el compañero tiene el mando (modo consulta, fila 24) o si
`guias.json` todavía no se ha leído (se reintenta sola en el repintado siguiente, nunca se marca
el asunto como "ya probado").

**El reparto.** `js/hitos-panel.js` pasa a pintar él solo el bloque entero (antes solo
sustituía el contenido de `#ficha-guia` cuando ya había hitos, y dejaba la guía de siempre para
un asunto sin ellos); `js/ficha-asunto.js` pierde su función `pintarGuia` entera. De paso, y
porque el encargo lo pedía (el fichero pasaba de 1.100 líneas), se ha sacado a
`js/ficha-contacto.js` el contacto del tercero y sus otros asuntos, sin tocar cómo se ven.

No se ha tocado `js/guias.js` ni `js/guias-enganche.js`: la guía se sigue escribiendo y
enseñando igual, como recordatorio al crear un asunto (`#guia-nuevo`) y en el propio cuadro de
escribirla; lo único que cambia es cómo se consume dentro de un asunto ya abierto.

`pruebas/guias.mjs` y `pruebas/opciones.mjs`, que antes comprobaban la guía como texto con
casillas, se han reescrito contra los hitos que nacen de ella; `pruebas/hitos.mjs` cambia su
escenario 2 (el asunto viejo ya no necesita ningún botón) y suma una prueba de que abrir dos
veces seguidas la misma ficha no duplica nada. Batería completa en verde. Versión publicada:
`App.VERSION`: `17-sep-2026 · 15:35`.
...
- **En `js/lector.js`** (el panel de leer un correo): al abrir un PDF se lee el pie sin retrasar
  el panel, y si aparecen código y dirección salen "Copiar el código" (con el código en gris al
