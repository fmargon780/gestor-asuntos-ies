...
## 26-sep-2026 — Fila 177: el índice del ARCHIVO por curso, y el tope de largo cuenta la ruta

`docs/ARCHIVO-POR-CURSO-Y-RUTAS.md`, segunda parte de la «tanda de estabilidad». Con unos 50
asuntos abiertos y más de 200 archivados en cuatro días, `_GESTOR/indice-archivo.json` (un solo
fichero con todos los asuntos juntos, reescrito entero cada vez que se archiva algo) crecería
varios megabytes en pocos cursos; y los topes de largo de nombre (`TOPE_ASUNTO`/`TOPE_DOCUMENTO`)
solo miraban el nombre, no la ruta completa dentro de Dropbox, que en Windows deja de sincronizar
sin avisar pasados los 260 caracteres.

1. **El índice, partido por curso académico**: `_GESTOR/indice-archivo.json` pasa a ser un
   resumen pequeño (`{ version, hechoEl, hechoPor, cursos, recuento }`); el índice de verdad vive
   en `_GESTOR/indice-archivo/<curso>.json`, uno por curso, calculado de las seis primeras cifras
   del nombre de la carpeta (`U.cursoDeFecha`, la misma cuenta de siempre). `leerDisco()` sin
   argumentos trae solo el curso actual; `leerDisco({ curso })`, uno concreto;
   `leerDisco({ todos: true })`, todos juntos. `anadirEntrada`/`quitarEntrada` escriben solo el
   fichero de su curso. Migración sin manos la primera vez que se lee un resumen del formato
   antiguo (el viejo se aparta a `_GESTOR/copias/indice-archivo-antiguo-AAMMDD.json`).
   `js/archivo-indice.js` (435 líneas) se partió en dos: leer/guardar/añadir/quitar se quedan ahí,
   y recorrer el disco (`construir`, `recuentoActual`) pasa a `js/archivo-indice-construir.js`
   (mismo `IndiceArchivo`, con `Object.assign`).
2. **Quién pide `todos: true`, y por qué**: Cuentas (que ya filtra por curso encima, sobre lo
   cargado), fichas huérfanas, las sugerencias de «Por clasificar» del mismo tipo, un relacionado
   archivado, quién ya tiene un asunto de un tipo (Repartir y Ajustes → borrar tipo) y el plazo de
   conservación. Todos necesitan ver el archivo entero, no solo el curso de hoy.
3. **Pantalla ARCHIVO**: desplegable «Curso: 2026-27 ▾» junto al buscador, con los cursos del
   resumen más «Todos los cursos»; por defecto, el actual. La ficha de una persona ("Sus asuntos")
   no pasa por el índice y sigue viendo todos sus archivados de cualquier curso, sin tocar nada.
4. **El tope de largo cuenta la ruta**: `Nombres.topes(tercero, categoria)` (`js/nombres-topes.js`,
   nuevo) calcula el hueco real a partir de la ruta de ARCHIVO señalada en `_GESTOR/rutas.json`
   (`RutaCarpetas.comunConocido`, getter nuevo en `js/copiar-ruta.js`), la categoría y el tercero,
   con tope total 240; sin `rutas.json` señalado, los 150/120 fijos de siempre (se lee ya al
   entrar, en `js/nucleo.js`, para que esté disponible desde el primer nombre que se proponga).
   `montarAsunto`/`montarDocumentoAjustado` devuelven además `noCabe` cuando ni recortando el
   texto libre por completo cabe; `Nombres.avisoRecorte(el, recortado, noCabe)` pinta ámbar o
   rojo («El nombre no cabe en la ruta de Dropbox: acorta el texto.»), y Nuevo asunto, Editar y
   poner nombre a un documento apagan su botón de guardar y lo comprueban otra vez antes de
   escribir, por si acaso.
5. **Lo que costó de verdad**: `topesDeHoy` (dentro de `nombres.js`) llamaba a `window.Nombres`
   sin comprobar que `window` existiera; `pruebas/nombre-corto-de-tipo.mjs` (una prueba sin
   navegador, con un contexto `vm` sin ningún `window`) lo cazó con un `ReferenceError` en cuanto
   `montarAsunto` pasó a llamar a `Nombres.topes()`. Arreglado con `typeof window !== 'undefined'`
   antes de mirar `window.Nombres`. Varios avisos "ámbar de más" o pantallas en blanco que parecían
   regresiones de esta fila, al mirarlos de cerca, eran del entorno de pruebas de esta sesión (sin
   `git push`, sin acceso a `raw.githubusercontent.com`): faltaban `js/lib/pdf-lib.min.js`,
   `js/lib/pdf.min.mjs`/`pdf.worker.min.mjs`, `datos-biblioteca/biblioteca-centro.json` y
   `scripts/plantillas-copia/`, ninguno descargado para esta prueba local por ser binarios grandes
   ajenos a esta fila; no son un fallo del cambio.

Pruebas: `pruebas/archivo-por-curso.mjs`, nueva y sin navegador (migración del formato antiguo,
`anadirEntrada` por curso, y `Nombres.topes()` con y sin `rutas.json`). `pruebas/archivo-indice.mjs`
y `pruebas/plazo-de-conservacion.mjs` se actualizaron para el índice por curso: fechas de los
asuntos de la prueba calculadas desde `U.cursoActual()` (nunca a mano, para que no se salgan de
curso solas con el paso del tiempo) y `leerDisco({ todos: true })` donde hacía falta ver más de un
curso. `npm test` completo, con la copia local del repositorio reconstruida fichero a fichero (esta
sesión no tiene `git push`): sin fallos atribuibles al cambio; los que salen son por las piezas
binarias que no se pudieron traer a esa copia local (arriba).
