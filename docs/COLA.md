...
| 11 | `docs/CORREOS-AL-ASUNTO.md` | EN CURSO | Empezada el 16-sep-2026. Elegir a mano el asunto de destino de un correo, con lista de "Podrían encajar"; huella del hilo en `asuntos.json`; el recolector de Apps Script sigue los hilos ya conocidos para que entren y salgan respuestas. Acordado con Francisco el 16-sep-2026. |
| 12 | `docs/DOCUMENTO-A-ASUNTO-EXISTENTE.md` | HECHA (código) — pendiente de fusionar a `main` | Botón "Meter en un asunto" en Por clasificar, con el elegidor compartido `js/elegir-asunto.js` (la fila 11 seguía EN CURSO de otra sesión, así que nace en solitario; que la 11 lo reutilice al retomarse). Batería completa en verde (`npm test`, 20 ficheros). **Esta sesión no ha podido subir directamente a `main`**: el entorno de ejecución exige rama y pull request (`claude/pending-queue-tasks-of0vyn`); hace falta que alguien lo fusione para que Vercel lo publique. Detalle en `docs/HISTORIA.md`. |
| 13 | `docs/ADJUNTAR-DOCUMENTOS-AL-CORREO.md` | EN CURSO | Empezada el 16-sep-2026 (la fila 11 sigue EN CURSO de otra sesión; la propia instrucción dice que sin `hilos` todo sale como correo nuevo y nada falla, así que se hace igual). |
