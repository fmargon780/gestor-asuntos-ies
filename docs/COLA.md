...
| 12 | `docs/DOCUMENTO-A-ASUNTO-EXISTENTE.md` | HECHA (código) — pendiente de fusionar a `main` | **Ojo: otra sesión, sin ver esto, ha vuelto a marcar esta fila EN CURSO directamente en `main` el 16-sep-2026** (no puede verlo: esta nota vive en el pull request, todavía sin fusionar). Está hecha de verdad, aquí: botón "Meter en un asunto" en Por clasificar. La fila 11 seguía EN CURSO de otra sesión al empezar esta, así que el elegidor nace en su propio módulo, `js/elegir-asunto.js`, en vez de en `js/bandeja-enlace.js` (que la fila 11 terminó mientras tanto, por su cuenta): **quedan dos elegidores de asunto en paralelo, sin unificar** (ver "Qué falta por hacer" en `docs/CONTEXTO.md`). Batería completa en verde. **Esta sesión no ha podido subir directamente a `main`**: el entorno de ejecución exige rama y pull request, **https://github.com/fmargon780/gestor-asuntos-ies/pull/7**; hace falta fusionarlo para que Vercel lo publique y para que se vea que esta fila ya no está libre. Detalle en `docs/HISTORIA.md`. |
| 13 | `docs/ADJUNTAR-DOCUMENTOS-AL-CORREO.md` | HECHA (código) — pendiente de fusionar a `main` | Bloque "Documentos de este asunto" en el cuadro de Correo, con casillas y límite de 20 MB; el script de Apps Script monta un borrador en Gmail con los adjuntos, respondiendo dentro del hilo si lo hay. Tarjeta "Borrador en camino" en Asuntos abiertos, con vigilancia cada 15 segundos. Batería completa en verde. Misma situación que la fila 12: pendiente de fusionar **el pull request https://github.com/fmargon780/gestor-asuntos-ies/pull/7**. Detalle en `docs/HISTORIA.md`. |
| 14 | `docs/PLANTILLAS-DE-CORREO.md` | HECHA (código) — pendiente de fusionar a `main` | Plantillas por tipo de asunto, con huecos que se rellenan solos, para el correo y para el mensaje de Séneca; desplegable en los dos cuadros, aviso de huecos sin datos, recorte a 4.000 letras en Séneca. Bloque propio en Ajustes, con alta, edición, borrado y vista previa; firma y centro editables, ya no escritos a mano en el código. Batería completa en verde. Misma situación que las filas 12 y 13: pendiente de fusionar **el pull request https://github.com/fmargon780/gestor-asuntos-ies/pull/7**. Detalle en `docs/HISTORIA.md`. |
