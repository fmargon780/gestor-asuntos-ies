...
| 18 | `docs/CORREO-EN-DOS-BUZONES.md` | PENDIENTE | Acordado con Francisco el 17-sep-2026. Un mismo correo en los dos buzones: se guarda la matrícula del mensaje (el `Message-ID`, igual en todos los buzones), la bandeja saca una línea gris "ya está en el asunto X, lo metió Juan" cuando el correo ya lo guardó el otro, y las respuestas del hilo vuelven a los dos buzones. Toca también `apps-script/gestor-correos.gs`: al terminar hay que decirle a Francisco que vuelva a pegarlo en `script.google.com`. |
...
| 23 | `docs/REFRESCO-DE-PANTALLA.md` | HECHA | **La primera de todas.** La lógica de repintado ya estaba bien (comprobado con el disco de mentira: las tres acciones refrescaban solas); el fallo real es que nada avisaba de que se estaba guardando, y con la carpeta de verdad en Dropbox el guardado tarda lo bastante para parecer que no ha hecho nada. `U.mientrasGuarda(control, fn)` nuevo (`js/util.js`): apaga el control ("Guardando…" en un botón) mientras se guarda. Puesto en estado, vía, plazo, archivar/reabrir y en marcar/cambiar de rama/responsable/fecha/notas/documentos de un hito. Prueba nueva `pruebas/refresco.mjs`. Batería completa en verde. Detalle en `docs/HISTORIA.md`. |
| 24 | `docs/NO-PISARSE-EN-UN-ASUNTO.md` | PENDIENTE | **La tercera, después de la 25.** Acordado con Francisco el 17-sep-2026. Señal de presencia en `_GESTOR/presencia.json`: al abrir la ficha de un asunto se apunta quién está dentro, se renueva cada 30 segundos y caduca a los 3 minutos. Si el otro ya está dentro, se entra **en modo consulta** (se ve todo, los botones que modifican salen apagados) con aviso arriba y botón "Tomar el mando". Marca en la tarjeta de la lista. Descartado, por ahora, montar una base de datos en internet para que el aviso fuera instantáneo. No toca el script de Apps Script. |
| 25 | `docs/POR-CLASIFICAR-DOCUMENTO-A-LA-VISTA.md` | PENDIENTE | **La segunda, después de la 23.** Acordado con Francisco el 17-sep-2026. En "Por clasificar", el documento abierto en el panel de la derecha queda marcado en la lista de la izquierda (fondo y barra de color), la lista se desplaza sola hasta él, y su nombre completo sale en la cabecera del panel. Además, los tres botones del documento (crear asunto, meter en un asunto, borrar) pasan a estar también dentro del panel de la derecha, compartiendo código con la tarjeta. No toca el script de Apps Script. |
