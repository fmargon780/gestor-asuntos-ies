...
(function () {
  ...
  async function guardar() {
    ...
    /* fila 77, docs/DETALLES-DE-MANTENIMIENTO.md, punto 3: respeta un
       "Quitar" hecho desde el otro ordenador aunque este todavía lo
       tenga en memoria. No hace falta Borrados.revivir en la alta: el
       id de un recurrente nuevo siempre es fresco ('r' + Date.now()),
       nunca reutiliza uno ya borrado. */
    lista = await Borrados.filtrarActivos(g, 'recurrentes', lista, function (r) { return r.id; });
  ...
  function filaDeRecurrente(r, alRefrescar) {
    ...
    quitar.onclick = async function () {
      ...
      var g = window.Gestor.carpetaGestor();
      if (g) await Borrados.marcar(g, 'recurrentes', r.id);
      await guardar();
      refrescarTodo();
